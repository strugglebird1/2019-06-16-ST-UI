          ...
          <el-table-column label="Operation" width="160" align="center">
            <template slot-scope="scope">
              ...
              <span v-else style="cursor: pointer;">
                <svg-icon
                  v-if="scope.row.status==='Normal'&&(scope.row.name !=='custUserId')"
                  icon-class="jinyong"
                  class="idel"
...
<script>
import { getschemalist, disableColumn, enableColumn } from "@/api/getlist";
import { setColumn } from "@/api/creat";
export default {
  data() {
    return {
      generateId: {
    ...
    getschemalist() {
      getschemalist().then(Response => {
        if (Response.data.code === 0) {
          this.master_user.data = Response.data.data.schema[0];
    ...
    pwdChange(row, index, cg) {
      ...
      if (row.isSet) {
        if (this.master_user.sel.name === "") {
          ...
          field.default = this.master_user.sel.default;
          data.fields = [];
